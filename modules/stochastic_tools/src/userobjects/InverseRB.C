...
InverseRB::validParams()
{
  ...
  params.addClassDescription(
      "Evaluates a trained DEIM reduced-basis model at a new parameter sample by solving the "
      "reduced nonlinear system with a Newton iteration.");
...
InverseRB::initialSetup()
{
  ...
  // Cache each residual component's selection indices, keyed by component name, from the mapping.
  for (const auto & component : _mapping->getResidualComponents())
    _residual_inds[component] = _mapping->getResidualSelectionIndices(component);
  ...
  // Union of every residual component's selection indices. This set avoids duplicate entries.
  std::set<dof_id_type> residual_dofs_set;
  for (const auto & [_, component_inds] : _residual_inds)
    residual_dofs_set.insert(component_inds.begin(), component_inds.end());

  std::vector<dof_id_type> residual_vec_inds(residual_dofs_set.begin(), residual_dofs_set.end());

  // Create a set to store the total indices
  std::set<dof_id_type> total_set;

  // Insert the residual union indices into the total_set
  total_set.insert(residual_vec_inds.begin(), residual_vec_inds.end());
  ...
  _red_res_elem = findReducedElemRange(residual_vec_inds);
  ...
  _red_res_node = findReducedNodeRange(residual_vec_inds);
...
std::map<VariableName, std::vector<Real>>
InverseRB::getReducedResValues()
{
  // Residual component names are either the literal "residual" (N=1 legacy, read from the total
  // residual RHS) or "residual::<tag>" (read from that tag's assembled vector).
  const std::string residual_tag_prefix = "residual::";

  std::map<VariableName, std::vector<Real>> red_res_values_per_component;

  for (const auto & component : _mapping->getResidualComponents())
  {
    mooseAssert(component == "residual" ||
                    component.compare(0, residual_tag_prefix.size(), residual_tag_prefix) == 0,
                "Residual component '" + component +
                    "' is neither 'residual' nor 'residual::<tag>'.");

    // Resolve this component's source vector from the single full assembly.
    const NumericVector<Number> & res =
        component == "residual"
            ? _nl_sys.RHS()
            : _nl_sys.getVector(
                  _fe_problem.getVectorTagID(component.substr(residual_tag_prefix.size())));

    const auto & inds = libmesh_map_find(_residual_inds, component);

    std::unordered_map<dof_id_type, Real> res_map;
    const auto local_start = res.first_local_index();
    const auto local_end = res.last_local_index();
    for (const auto & index : inds)
      if (index >= local_start && index < local_end)
        res_map[index] = res(index);

    // Root has the correct map now
    _communicator.set_union(res_map);

    auto & red_res_values = red_res_values_per_component[component];
    red_res_values.reserve(inds.size());
    for (const auto & index : inds)
      red_res_values.push_back(libmesh_map_find(res_map, index));

    _communicator.broadcast(red_res_values);
  }

  return red_res_values_per_component;
}
DenseMatrix<Real>
...
{
  // A single full assembly populates the total residual (RHS) and every active residual tag vector;
  // each residual component then reads its own selected DOFs from this one assembly.
  _fe_problem.computeResidual(*_curr_sol, *_residual, 0);
  const auto red_res_values_per_component = getReducedResValues();
  return _mapping->compute_reduced_res(red_res_values_per_component);
}
