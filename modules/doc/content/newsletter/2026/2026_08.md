...
### Domain decomposition preconditioning (BDDC / FETI-DP)

The new [`DDP`](DomainDecompositionPreconditioner.md) preconditioner assembles the Jacobian in
PETSc's globally unassembled `MATIS` format and solves with PETSc's non-overlapping domain
decomposition solvers: PCBDDC for the primal balancing method, or KSPFETIDP for the dual-primal
method. The decomposition places one subdomain per MPI process, and the preconditioner applies the
correctness-critical PETSc defaults itself.
...
- The new [RigidBodyModes](RigidBodyModes.md) user object fills the framework near-null-space
  vectors with the rigid body modes of the displacement field, plus one constant mode per coupled
  field, which accelerates algebraic multigrid on under-constrained (floating) solves. The
  QuasiStatic physics enables it with `rigid_body_near_null_space = true`.
