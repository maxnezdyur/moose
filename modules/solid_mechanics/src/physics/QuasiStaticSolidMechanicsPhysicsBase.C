...
std::map<std::string, std::string>
    QuasiStaticSolidMechanicsPhysicsBase::_rank_two_cartesian_component_table = {
        {"strain", "total_strain"},
...
const std::map<std::string, std::pair<std::string, std::vector<std::string>>>
    QuasiStaticSolidMechanicsPhysicsBase::_rank_two_invariant_table = {
...
const std::map<std::string, std::pair<std::string, std::vector<std::string>>>
    QuasiStaticSolidMechanicsPhysicsBase::_rank_two_directional_component_table = {
        {"directional", {"Direction", {"stress", "strain"}}}};

const std::map<std::string, std::pair<std::string, std::vector<std::string>>>
    QuasiStaticSolidMechanicsPhysicsBase::_rank_two_cylindrical_component_table = {
        {"axial",
...
const std::map<std::string, std::pair<std::string, std::vector<std::string>>>
    QuasiStaticSolidMechanicsPhysicsBase::_rank_two_spherical_component_table = {
        {"spherical_hoop",
         {"HoopStress", {"stress", "strain", "plastic_strain", "creep_strain", "elastic_strain"}}},
...
QuasiStaticSolidMechanicsPhysicsBase::validParams()
{
  ...
  params.addParam<bool>(
      "reduced_integration",
      false,
      "Generate hourglass stabilization kernels for one-point (reduced) quadrature. This flag does "
      "not set the quadrature rule; request it with [Executioner][Quadrature].");
  params.addRangeCheckedParam<Real>(
      "hourglass_penalty",
      0.1,
      "hourglass_penalty >= 0",
      "Dimensionless hourglass coefficient forwarded to the 'penalty' of each "
      "HourglassStabilization kernel generated by 'reduced_integration'");
  params.addParam<bool>(
  ...
  params.addParamNamesToGroup("strain incremental use_finite_deform_jacobian eigenstrain_names "
                              ...
                              "reduced_integration hourglass_penalty",
                              "Strain");
  ...
  : Action(parameters), _use_ad(getParam<bool>("use_automatic_differentiation"))
{
  ...
  auto action = _awh.getActions<CommonSolidMechanicsAction>();
  if (action.size() == 1)
...
void
QuasiStaticSolidMechanicsPhysicsBase::addCartesianComponentOutput(const std::string & enum_name,
