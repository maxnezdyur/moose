...
DensityUpdate::validParams()
{
  ...
  params.addParam<VariableName>("cost_density_sensitivity",
  ...
  params.addParam<Real>("cost_fraction", 1.0, "Cost fraction.");
  ...
  params.addParam<VariableName>("cost", "Name of the cost variable.");
...
DensityUpdate::DensityUpdate(const InputParameters & parameters)
  : ElementUserObject(parameters),
    ...
    _density_sensitivity(
        &_subproblem.getStandardVariable(_tid, getParam<VariableName>("density_sensitivity"))),
    _cost_density_sensitivity(isParamValid("cost_density_sensitivity")
                                  ? &_subproblem.getStandardVariable(
                                        _tid, getParam<VariableName>("cost_density_sensitivity"))
                                  : nullptr),
    _cost(isParamValid("cost")
              ? &_subproblem.getStandardVariable(_tid, getParam<VariableName>("cost"))
              : nullptr),
    ...
    _thermal_sensitivity(isParamValid("thermal_sensitivity")
                             ...
                             : nullptr),
    _thermal_compliance(getParam<bool>("use_thermal_compliance"))
{
  if (_thermal_compliance)
  {
  ...
  if (isParamValid("cost_density_sensitivity") &
      ...
      ElementData data = ElementData(
          ...
          _cost_density_sensitivity
              ? dynamic_cast<const MooseVariableFE<Real> *>(_cost_density_sensitivity)
                    ->getElementalValue(elem)
              : 0,
          _thermal_compliance ? _thermal_sensitivity->getElementalValue(elem) : 0.0,
          _cost ? _cost->getElementalValue(elem) : 0.0,
          elem->volume(),
          0);
  ...
  while (perform_loop)
  {
    ...
    for (auto && [id, elem_data] : _elem_data_map)
    {
      ...
      Real new_density = computeUpdatedDensity(
          ...
          elem_data.sensitivity * (_thermal_compliance ? _weight_values[0] : 1.0),
    ...
    // Determine whether to continue the loop based on the relative difference
    // between l1 and l2, if no cost exists reverts to volume constraint only
    perform_loop = (l2 - l1) / (l1 + l2) > _relative_tolerance ||
                   (_cost ? (c2 - c1) / (c1 + c2) > _relative_tolerance : false);
  }
}

// Method to compute the updated density for an element
Real
DensityUpdate::computeUpdatedDensity(Real current_density,
                                     Real dc,
                                     ...
                                     Real cmid)
{
  ...
  //                                std::max(1e-5, current_density) *
  //                                    std::sqrt((-(dc + temp_sensitivity) / denominator))))));
